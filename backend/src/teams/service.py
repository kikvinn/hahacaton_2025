...
class TeamService:
    ...
    @staticmethod
    def update_team(
        db: Session, team_id: int, team_update: schemas.TeamUpdate
    ...
    def add_team_member(db: Session, team_member: schemas.TeamMemberCreate) -> models.TeamMember:
        db_team_member = models.TeamMember(
            team_id=team_member.team_id, athlete_id=team_member.athlete_id, role=team_member.role
        )
    ...
    def remove_team_member(db: Session, team_id: int, athlete_id: int) -> bool:
        db_team_member = (
            ...
            .filter(
                models.TeamMember.team_id == team_id, models.TeamMember.athlete_id == athlete_id
            )
            .first()
        )
        if db_team_member:
    ...
    def get_team_members(db: Session, team_id: int) -> List:
        ...
        return (
            db.query(Athlete)
            .join(models.TeamMember, Athlete.id == models.TeamMember.athlete_id)
